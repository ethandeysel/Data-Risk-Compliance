...
- requirements: list EVERY legally operative statement as its own object — each obligation, prohibition, permission, condition, notification duty, safeguard, record-keeping rule or cross-border condition. Be exhaustive: a substantive section usually yields several. If the section enumerates or lists items (a set of strategies, controls, principles, categories, conditions or steps), extract EACH listed item as its own requirement — never collapse a list into a single entry or into the summary only. Do NOT merge distinct statements, and do NOT pad with non-operative narration. Each text is a clear, self-contained paraphrase (or short quote) of the rule. obligation_type is ONE of: {", ".join(OBLIGATION_TYPES)}. Use [] only when the section is purely definitional or narrative with no operative rule.
