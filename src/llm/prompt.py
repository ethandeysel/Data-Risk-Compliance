...
- requirements: list EVERY legally operative statement as its own object — each obligation, prohibition, permission, condition, notification duty, safeguard, record-keeping rule or cross-border condition. Be exhaustive: a substantive section usually yields several. If the section enumerates or lists items (a set of strategies, controls, principles, categories, conditions or steps), extract EACH listed item as its own requirement — never collapse a list into a single entry or into the summary only. Do NOT merge distinct statements, and do NOT pad with non-operative narration. Each text is ONE concise line — a single self-contained rule of at most ~20 words (a crisp paraphrase, not a long quote). obligation_type is ONE of: {", ".join(OBLIGATION_TYPES)}. Use [] only when the section is purely definitional or narrative with no operative rule.
